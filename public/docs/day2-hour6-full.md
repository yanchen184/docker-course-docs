...
容器內的檔案系統是隔離的。如果資料只存在容器內，一旦容器被刪除，資料就跟著消失。Volume（數據卷）技術就是把容器內的目錄「掛載」到宿主機上，實現雙向同步：
- 容器內新增或修改檔案 → 宿主機對應目錄自動同步
- 宿主機修改檔案 → 容器內也會同步更新
- 即使容器停止後再啟動，資料依然保持同步
...
即使把 docker01 刪除，docker02 和 docker03 的資料不會丟失。數據卷的生命周期持續到沒有任何容器在使用為止。容器之間是雙向拷貝的關係——只要有一個容器還活著，資料就在。

**典型應用場景**

```bash
# 多個 MySQL 實例共享數據
docker run -d --name mysql01 -v mysql-data:/var/lib/mysql mysql:5.7
docker run -d --name mysql02 --volumes-from mysql01 mysql:5.7
```

適用於集群配置共享、多實例數據同步等場景。如果第一個容器已經用 `-v` 把資料持久化到了宿主機，那宿主機上的資料永遠不會被刪除。
